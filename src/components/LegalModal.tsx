...
export const LegalModal = ({ title, onClose, children }: LegalModalProps) => {
  ...
  return (
    ...
    >
      ...
      />
      {/* Panel */}
      <div data-testid="legal-modal-panel" className="relative z-10 bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-8 py-5 border-b border-slate-100">
          <h2 className="text-xl font-bold text-sky-900">{title}</h2>
          <button
            onClick={onClose}
            data-testid="legal-modal-btn-close-header"
        ...
        <div className="px-8 py-4 border-t border-slate-100 flex justify-end">
          <button
            onClick={onClose}
            data-testid="legal-modal-btn-close-footer"
