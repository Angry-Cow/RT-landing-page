...
type Category = "All" | "First Aid" | "Personal Defense" | "Personal Awareness";

const CATEGORIES: Category[] = ["All", "First Aid", "Personal Defense", "Personal Awareness"];
...
const COURSES: Course[] = [
  {
    ...
    description: "A comprehensive seminar focused on situational awareness and personal safety strategies.",
  ...
  },
  {
    ...
    description: "Comprehensive first aid, CPR, and AED training. Gain certification-ready skills for responding to medical emergencies.",
    buttonText: "Contact Us Now To Schedule",
    category: "First Aid",
  },
  {
    ...
    description: "Comprehensive CPR and AED training for professional rescuers, EMTs, Nurses, other healthcare providers and lifeguards.",
    buttonText: "Contact Us Now To Schedule",
    category: "First Aid",
  },
  {
  ...
  },
  {
    ...
    description: "Traumatic Bleeding Skills for Emergency Responders, Law Enforcement, Security, beyond Stop The Bleed with significant hands on practice on simulated bleeding wounds. Chest seal application and review of IFAK contents and requirements.",
    buttonText: "Contact Us Now To Schedule",
    category: "First Aid",
  },
];
...
export const CoursesSection = () => {
  ...
  const { data: dbCourses, isPending, error } = useQuery("Course", {
    where: { switch: 1 },
    orderBy: { order: "asc" },
  });
  ...
  const activeCourses: Course[] =
    dbCourses && dbCourses.length > 0
      ? dbCourses.map((c) => ({
          ...
          privatePrice: c.privatePrice ?? undefined,
          privatePriceNote: c.privatePriceNote ?? undefined,
          features: Array.isArray(c.features)
            ? (c.features as unknown as string[])
            : (c.features ?? "").split(",").map((f: string) => f.trim()).filter(Boolean),
  ...
  const categoryCounts: Record<Category, number> = {
    ...
    "First Aid": activeCourses.filter((c) => c.category === "First Aid").length,
    "Personal Defense": activeCourses.filter((c) => c.category === "Personal Defense").length,
    "Personal Awareness": activeCourses.filter((c) => c.category === "Personal Awareness").length,
  };
  ...
  return (
    <section className="bg-slate-50 box-border caret-transparent py-24">
      <div className="box-border caret-transparent max-w-none w-full mx-auto px-6 md:max-w-screen-xl">
              ...
              >
                ...
                <span
                  className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-xs font-bold transition-colors duration-200 ${
                    isActive ? "bg-amber-500 text-white" : "bg-slate-100 text-gray-500"
                  }`}
                >
                  {categoryCounts[cat]}
                </span>
        ...
        {isPending && (
          <p className="text-center text-gray-400 py-8">Loading courses…</p>
        )}
        {error && (
          <p className="text-center text-red-400 py-8">
            Could not load courses. Showing default content.
          </p>
        )}
