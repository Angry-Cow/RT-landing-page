...
export const ServiceCard = (props: ServiceCardProps) => {
  return (
    <div className="box-border caret-transparent flex flex-col min-h-[auto] min-w-[auto]">
      <div className="bg-slate-50 box-border caret-transparent h-full min-h-[auto] min-w-[auto] border border-gray-100 p-8 rounded-3xl border-solid">
        ...
        <button
          ...
          data-testid={`service-card-btn-${props.title
            .toLowerCase()
            .replace(/\s+/g, "-")
            .replace(/[^a-z0-9-]/g, "")
            .slice(0, 40)}`}
          className="text-sky-900 font-bold items-center bg-transparent caret-transparent gap-x-2 flex gap-y-2 text-center p-0 hover:text-amber-600 transition-colors"
        >
          <span className="box-border caret-transparent block min-h-[auto] min-w-[auto]">
            {props.buttonText}
          </span>
      ...
      <div className="shadow-[rgba(0,0,0,0)_0px_0px_0px_0px,rgba(0,0,0,0)_0px_0px_0px_0px,rgba(0,0,0,0.1)_0px_10px_15px_-3px,rgba(0,0,0,0.1)_0px_4px_6px_-4px] box-border caret-transparent min-h-[auto] min-w-[auto] mt-6 rounded-3xl">
        <img
          ...
          className="block box-border caret-transparent w-full rounded-3xl"
        />
      </div>
    </div>
  );
